...
class WCLClient:
    ...
    async def character(self, name: str, realm: str, region: str, raid_id: str, difficulty_id: int,
        ...
        for item in raw_rankings:
            # Warcraft Logs may return encounter as either a string or an
            # object like {"id": 3176, "name": "Imperator Averzian"}.
            raw_boss_name = item.get("encounter", item.get("boss", "Босс"))
            if isinstance(raw_boss_name, dict):
                boss_name = (
                    raw_boss_name.get("name")
                    or raw_boss_name.get("label")
                    or raw_boss_name.get("title")
                    or "Босс"
                )
            else:
                boss_name = str(raw_boss_name or "Босс")
...
def build_raid_embed(raid: sqlite3.Row, players: list[dict]) -> discord.Embed:
    ...
    for p in confirmed:
        for boss in p.get("bosses", []):
            raw_boss_name = boss.get("boss") if isinstance(boss, dict) else boss
            # Old database rows may contain the whole encounter object as a
            # dict or its Python repr. Always render only the boss name.
            if isinstance(raw_boss_name, dict):
                boss_name = (
                    raw_boss_name.get("name")
                    or raw_boss_name.get("label")
            ...
            elif isinstance(raw_boss_name, str) and raw_boss_name.lstrip().startswith("{"):
                try:
                    parsed = ast.literal_eval(raw_boss_name)
                    if isinstance(parsed, dict):
                        boss_name = (
                            parsed.get("name")
                            or parsed.get("label")
                        ...
                        boss_name = raw_boss_name
                except (ValueError, SyntaxError):
                    boss_name = raw_boss_name
            else:
                boss_name = str(raw_boss_name or "Босс")
            boss_name = boss_name.strip()
            percentile = percentile_number(boss.get("percentile") if isinstance(boss, dict) else None)
...
class RaidSetupModal(discord.ui.Modal, title="Настройка рейда"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Notify everyone only once, when the raid is initially created.
        # The server must allow this bot/role to mention @here.
